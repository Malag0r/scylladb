    ...
    if (_next_row_in_range) {
        ...
        if (_read_context.tombstone_gc_state() // do not compact rows when tombstone_gc_state is not set (used in some unit tests)
            ...
            if (row_tomb_expired(t) || is_row_dead(row)) {
                ...
                _read_context.cache()._tracker.on_row_compaction_attempt();

                try {
                    with_allocator(_snp->region().allocator(), [&] {
                        deletable_row row_copy(*_schema, row);
                        row_copy.compact_and_expire(*_schema, t.tomb(), _read_time, always_gc, _gc_before, nullptr);
